...
pred_dir = os.path.join(args.result_path, 'panoptic')
depth_dir = os.path.join(args.result_path, 'depth')
gt_dir = 'data/cityscapes-dvps/video_sequence/val/'
...
def main():
    ...
    print(
        '{:.2f} {:.2f} {:.2f}'.format(
            np.nan_to_num(pq).mean() * 100,
            np.nan_to_num(tpq).mean() * 100,
            np.nan_to_num(spq).mean() * 100))
    print(abs_rel)
